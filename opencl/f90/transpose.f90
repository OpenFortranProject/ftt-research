program transpose
   ...
   kernel = createKernel(device, &
                         "transpose.cl"  // C_NULL_CHAR, &
                         "simple_copy"   // C_NULL_CHAR)

   ! add arguments
   !
   status = setKernelArgMem(kernel, 0, clMemObject(d_dst))
   status = setKernelArgMem(kernel, 1, clMemObject(d_src))
   status = setKernelArgInt(kernel, 2, 0)
   ...
   print *, "throughput ==", throughput, "GB/s  ", "size ==", nxg*nyg, &
