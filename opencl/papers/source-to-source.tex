...
For example, {\tt wave_advance} is transformed as {\tt __kernel void
  wave_advance(__ global float * H, ..., float dt);}.  The global state
variables have local equivalents, e.g., {\tt __local float
  H_local[MAX_LOCAL_SIZE]}, declared with the appropriate size and are copied
