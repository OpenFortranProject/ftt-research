...
\begin{itemize}
   \item !$OFP CONTIGUOUS: specifies that an dummy array variable is
   contiguous in memory. CONTIGUOUS is an attribute in Fortran 2008.
   \item !$OFP KERNEL: specifies that a pure subroutine can be
...
\begin{itemize}
   \item transfer_halo(array, halo): an impure subroutine that
   ...
   \item interior(array, halo): a pure function that returns a copy of
     the interior portion of the array specified by halo. Array is
     local to a node.
   \item region(array, halo): a pure function that returns a copy of
...
% TODO - Matt - do you want a try at this.  Not sure we have room for
% this subsection. I briefly mention node parallelism in other sections.
...
A kernel procedure (specified by the !$OFP KERNEL directive) must be
