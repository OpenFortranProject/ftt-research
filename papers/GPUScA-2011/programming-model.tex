...
programming model in Fortran relates to developments in other languages.


\subsection{Comparison to Other Languages}

A number of previous efforts have exploited data-parallel programming at the
language level to utilize novel architectures, particularly in previous decades
during the reign of vector and massively parallel computers in high performance
computing (HPC).  The origin of the array syntax adopted by Fortran in the 1990
...
the expression level, instead of via parallelism within explicit DO-loops (such
as within IVTRAN for the Illiac IV).
...
to implement loops was left entirely up to the tool.  Unfortunately, this had
...
An interesting line of language research that grew out of the early work with
HPF was that associated with the ZPL language work at the University of
Washington~\cite{chamberlain04zpl}.  In ZPL, programmers adopt a similar global
view of computation over arrays, but define their computations based on regions,
which provide a local view of the set of indices that participate in the update
of each element of an array.  The Chapel parallel programming language being
developed by Cray is directly influenced by HPF and
ZPL~\cite{chamberlain_chapel}.

\subsection{Fortran Language Subset}

The static analysis and source-to-source transformations used in this work are
very basic and only require the programmer to use a language subset that employs
a data-parallel programming model.  In particular, it encourages the use of
array notation, elemental functions, and pure procedures.  From these language
constructs, we are able to easily transform Fortran procedures to a lower-level
...
\noindent results in the element-wise sum of {\tt A} and {\tt s} times the
...
accelerators in OpenCL.  Currently our prototype tool transforms pure procedures
...
In Fortran, these functions return a copy of an existing array or array section.
...
increased to provide extra array elements surrounding the interior portion of
the array.  This is critical to reducing the amount of code that the programmer
is forced to write, as the halo cells are often where boundary errors and
off-by-one errors may occur in code that is manually generated.
...
ForOpenCL provides two new procedures that are defined in Fortran and are used
...
\item {\tt region(array, halo)}: a pure function that returns a copy of
  the portion of the array specified by halo.

\end{itemize}

It should be noted that the two functions {\tt interior} and {\tt region} are
pure and thus can be called from within a pure kernel procedure.  These two
functions are part of the language recognized by the compiler and though the two
functions return a copy of a portion of an array \emph{semantically}, the
compiler is not forced to actually make a copy and is free to enforce copy
semantics through other means.  In addition to these two new functions,
ForOpenCL provides the compiler directive, {\tt \!\$OFP KERNEL}, which specifies
that a pure subroutine can be transformed to an OpenCL kernel.
...
similar to functional programming (or value-oriented programming).

Unfortunately, this style of programming has never really caught on because when
array syntax was first introduced in Fortran, performance of these features was
...
in this style of Fortran are maintainable and can achieve good performance on
accelerator architectures.

\subsection{Restrictions}

Only pure Fortran procedures are transformed into OpenCL kernels.  The
programmer must explicitly call these kernels from Fortran using the ForOpenCL
...
pure and all array variables must be declared as contiguous.  A kernel procedure
may not call other procedures except for limited intrinsic functions (primarily
math), user-defined elemental functions, and the {\tt interior} and {\tt region}
functions.  Future work will address non-contiguous arrays (such as those that result
from strided access) by mapping array strides to gather/scatter-style memory
accessors.
