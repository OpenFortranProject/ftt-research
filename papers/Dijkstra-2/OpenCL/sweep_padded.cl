...
__kernel void sweep_padded ( const int nx, const int ny
		           ...
		           , const int step)
{
  ...
  const int xSize = get_global_size(0) + 2*PAD;
  const int ySize = get_global_size(1) + 2*PAD;
  const int zSize = get_global_size(2) + 2*PAD;

  int i = get_global_id(0) + PAD;
  int j = get_global_id(1) + PAD;
  int k; // two-dimensional parallel decomposition */

  // make sure that the thread id is within the bounds of the array
  if (i >= nx+PAD || j >= ny+PAD)  return;
