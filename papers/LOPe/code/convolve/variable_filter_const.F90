...
contains
   ...
   subroutine convolve(S, Image, F)
      ...
      do j = 1, NY
         do i = 1, NX
            ...
            do jp = -NPAD, NPAD
               do ip = -NPAD, NPAD
                  val = val + F(1,1)%p(ip,jp)*Image(i+NPAD,j+NPAD)
               end do
            end do
            S(i,j) = val
         end do
   ...
   subroutine init_filter(F)
      ...
      print *, nxp, nyp, NPAD
      do i = -1, 1
         print "('F==',100(f5.3,1x))", F(1,1)%p(i,-1:1)
      end do
      print *
...
program test_variable_filter
   ...
   real(c_float),     target, allocatable  :: I(:,:)      ! image
   real(c_float),     target, allocatable  :: S(:,:)      ! smoothed image
   type(FilterPatch), target, allocatable  :: F(:,:)      ! filter
