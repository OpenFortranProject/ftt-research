...
!...TODO-END-GENERATE
   ...
   integer(c_size_t), parameter :: NY  = 256
   integer(c_size_t), parameter :: NZ  = 256
...
!...TODO-GENERATE
   ...
   ! work group size
   integer(c_size_t), parameter :: NXL = 64
   integer(c_size_t), parameter :: NYL = 2
   integer(c_size_t), parameter :: NZL = 2
...
!...TODO-END-GENERATE
   ...
   integer :: device, i, j, k, hoff, nLoops = 100, nWarm = 10
...
!   cl_C_ = createBuffer(cl_device_, CL_MEM_WRITE_ONLY, mem_size, C_NULL_PTR)
   ...
   print *, aContext%rank, ": host time ==", real(h_time)/nLoops, "ms per iteration"
      ...
      do j = 1, ny
         do i = 1, nx
            if (C(i,j,k) /= A(i,j,k) + B(i,j,k)) then
               print *, aContext%rank, ": Results incorrect at ", i, j, k
               goto 99
            end if
         end do
      end do
