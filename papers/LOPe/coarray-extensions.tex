...
between logically neighboring processes.\todo{What if corank and rank are not the same?}  For example, if the process
...
\todo{subimages are critical and deserve their own subsection}
...
devices\todo{mention the relation of subimages and hardware earlier in
  defn. of subimage}.  If the function fails (e.g., the requested
...
%% However, recall that with the LOPe programming model only the local element is mutable; thus, the halo region of an array needs update.
...
add\todo{Add?  is that typo?} memory deallocation is similar to memory allocation:
...
language allowing the compiler access to this information in order to spread computation over more
hardware resources, improve performance, and to reduce complexity for the programmer.
